...
async function checkToken(
  request: NextRequest,
  endpoint: "admin/check-token" | "donor/check-token",
): Promise<boolean> {
...
function isDonorProtectedPath(pathname: string): boolean {
  return donorProtectedPrefixes.some(
...
export async function middleware(request: NextRequest) {
  ...
  // Avoid showing login screens when a valid session already exists.
  if (isLoginPath) {
