...
const TestimonialsSection = () => {
    ...
    const renderStars = (rating: number) => Array.from({ length: 5 }, (_, i) => (<FaStar key={i} className={`${styles.star} ${i < rating ? styles.starFilled : styles.starEmpty}`}/>));
    return (<section className={`${styles.testimonialsSection} section`}>
      <div className="content">
        <h2 className="title">{t("testimonials.title")}</h2>
        <p className="subtitle">{t("testimonials.subtitle")}</p>
      </div>

      <Swiper modules={[Autoplay]} pagination={{ clickable: true }} autoplay={{ delay: 4000, disableOnInteraction: false }} spaceBetween={30} loop breakpoints={{
            320: { slidesPerView: 1 },
            768: { slidesPerView: 2 },
        ...
        {testimonials.map((testimonial, index) => (<SwiperSlide key={index}>
            <div className={styles.testimonialCard}>
              <div className={styles.testimonialHeader}>
                ...
                {testimonial.verified && (<span className={styles.verifiedBadge}>
