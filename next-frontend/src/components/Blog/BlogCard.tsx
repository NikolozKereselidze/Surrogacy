...
const BlogSkeleton = () => (<article className={styles.blogCard}>
    <div className={styles.blogImageContainer}>
      <div className={`${styles.skeleton} ${styles.skeletonImage}`}/>
      <div className={`${styles.skeleton} ${styles.skeletonCategory}`} style={{ position: "absolute", top: "1rem", left: "1rem" }}/>
    </div>
    ...
    <div className={styles.blogContent}>
      ...
      <div className={`${styles.skeleton} ${styles.skeletonLink}`} style={{ marginTop: "0.6rem" }}/>
    </div>
  </article>);
const BlogCard = () => {
    ...
    const postsWithFormattedDate = useMemo(() => blogPosts.map((p) => ({
        ...p,
    ...
    return (<section className={`${styles.blogSection} section`}>
      ...
      <div className={styles.blogGrid}>
        {loading ? (Array.from({ length: MAX_POSTS }, (_, i) => <BlogSkeleton key={i}/>)) : blogPosts.length > 0 ? (postsWithFormattedDate.map((post) => (<article key={post.id} className={styles.blogCard}>
              <div className={styles.blogImageContainer}>
                {post.imageUrl ? (<Image src={post.imageUrl} alt={post.title} className={styles.blogImage} width={500} height={500}/>) : (<div aria-hidden="true" style={{
                    width: 500,
                    height: 500,
                    background: "#f0f0f0",
                    borderRadius: 8,
              ...
              <div className={styles.blogContent}>
                ...
                <Link href={`/${post.language.toLowerCase()}/blog/${post.id}/${post.title
                .toLowerCase()
                .trim()
                .replace(/[^a-z0-9]+/g, "-")
                .replace(/^-+|-+$/g, "")}`} className={styles.readMoreLink}>
                  {t("blog.readMore")} →
                </Link>
              </div>
