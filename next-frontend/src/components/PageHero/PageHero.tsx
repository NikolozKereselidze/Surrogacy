...
const PageHero = ({ id, translationKey, sidebarVariant = "simple", sidebarId, primaryOnClick, primaryHref, secondaryHref, secondaryOnClick, className, }: PageHeroProps) => {
    ...
    const sidebarTitle = sidebarVariant === "detailed"
    ...
    const renderSecondaryCta = () => {
        if (!secondaryLabel)
            return null;
        if (secondaryHref) {
            return (<Link href={secondaryHref} className={styles.secondaryBtn}>
          {secondaryLabel}
        </Link>);
        }
        return (<button type="button" className={styles.secondaryBtn} onClick={secondaryOnClick}>
        {secondaryLabel}
      </button>);
    };
    return (<section className={`${styles.hero} ${className ?? ""}`.trim()} aria-labelledby={id}>
      <div className={styles.heroInner}>
        <div className={styles.heroContent}>
          ...
          <div className={styles.heroActions}>
            {primaryHref ? (<Button href={primaryHref}>
                {t(`${translationKey}.primaryCta`)}
              </Button>) : (<Button onClick={primaryOnClick}>
                {t(`${translationKey}.primaryCta`)}
              </Button>)}
            {renderSecondaryCta()}
          </div>
        ...
        <aside className={styles.sidebarCard} aria-labelledby={resolvedSidebarId}>
          <h2 id={resolvedSidebarId} className={styles.sidebarTitle}>
            {sidebarTitle}
          </h2>

          {sidebarVariant === "simple" ? (<ul className={styles.simpleList}>
              {cardPoints.map((point) => (<li key={point}>{point}</li>))}
            </ul>) : (<ul className={styles.highlightList}>
              {highlights.map((item) => (<li key={item.title} className={styles.highlightItem}>
