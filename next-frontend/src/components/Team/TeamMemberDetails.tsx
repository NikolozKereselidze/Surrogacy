...
const TeamMemberDetails = ({ id }: {
    id: string;
}) => {
    const router = useRouter();
    const locale = useLocale();
    const member = useMemo(() => teamMembers.find((m: TeamMember) => m.id === id), [id]);
    const personSchema = useMemo(() => member
        ? {
          ...
          <div className={styles.teamMemberInfo}>
            <div className={styles.teamMemberHeader}>
              ...
              <div className={styles.imageContainer}>
                <Image src={member.image} alt={`${member.honorific} ${member.name}`} className={styles.memberImage} width={108} height={108}/>
              </div>
            </div>
            ...
            <div className={styles.teamMemberContact}>
              ...
              <div className={styles.contactLinks}>
                <a href={`mailto:${member.email}`} className={styles.contactLink} aria-label={`Email ${member.name}`}>
                  <FaEnvelope className={styles.contactIcon}/>
                  <span>{member.email}</span>
                </a>
                <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className={styles.contactLink} aria-label={`LinkedIn profile of ${member.name}`}>
