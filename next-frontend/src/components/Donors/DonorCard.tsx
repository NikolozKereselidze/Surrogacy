          ...
          <div className={styles.donorCard}>
            ...
            <div className={styles.leftSection}>
              <div className={styles.mainImageContainer}>
                <Image src={`${CLOUDFRONT_DOMAIN}/${allImages[currentImageIndex]}`} alt={donor.id} className={styles.mainImage} width={800} height={800} onError={(e) => {
            console.error("Image failed to load:", e.currentTarget.src);
              ...
              <div className={styles.thumbnailGallery}>
                {allImages.map((imagePath, index) => (<Image key={index} src={`${CLOUDFRONT_DOMAIN}/${imagePath}?w=150&q=100&f=webp`} alt={`${donor.id} ${index + 1}`} width={150} height={150} className={`${styles.thumbnail} ${currentImageIndex === index ? styles.activeThumbnail : ""}`} onClick={() => setCurrentImageIndex(index)}/>))}
              </div>
            </div>
            ...
            <div className={styles.rightSection}>
              <div className={styles.donorAvailability}>
                <span className={donor.available ? styles.available : styles.notAvailable}>
              ...
              {donor.documentPath && (<div className={styles.donorDocument}>
                  <a className={styles.donorDocumentLink} href={`${CLOUDFRONT_DOMAIN}/${donor.documentPath}`} target="_blank" rel="noopener noreferrer">
