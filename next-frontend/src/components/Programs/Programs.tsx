...
const Programs = ({ programType }: ProgramsProps) => {
    ...
    const getProgramData = () => {
        ...
        return {
            ...
            questions: config.questions?.map((qa) => ({
    ...
    return (<section className={`${styles.gametesSection} section`}>
      <div className={styles.content}>
        <h1 className={styles.title}>{programData.title}</h1>
        <p className={styles.description}>{programData.description}</p>
      </div>

      {programData.sections.map((section, index) => (<div key={index} className={styles.content}>
      ...
      {programData.questions.length > 0 && (<div className={styles.faqSection}>
          <h2 className={styles.faqTitle}>{t("programs.commonQuestions")}</h2>
          <div className={styles.faqGrid}>
            {programData.questions.map((qa, idx) => (<div key={idx} className={styles.faqItem}>
