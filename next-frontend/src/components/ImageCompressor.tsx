...
const ImageCompressor: React.FC<ImageCompressorProps> = ({ onCompressed, onMultipleCompressed, multiple = false, maxWidth = 800, maxHeight = 800, quality = 0.7, label = "Upload Image", }) => {
    ...
    return (<>
      ...
      <input id={label} ref={inputRef} type="file" multiple={multiple} accept="image/*" onChange={handleChange}/>

      
      {!multiple && preview && (<div style={{ marginTop: "10px" }}>
          <strong>Compressed Preview:</strong>
          <br />
          <Image src={preview} alt="Compressed" width={120} height={120} style={{ maxWidth: "100%", maxHeight: "200px" }}/>
        </div>)}

      
      {multiple && previews.length > 0 && (<div style={{ marginTop: "10px" }}>
          ...
          <div style={{
            ...
            {previews.map((preview, index) => (<div key={index} style={{ position: "relative" }}>
                <Image src={preview} alt={`Compressed ${index + 1}`} style={{
                    maxWidth: "100px",
                    maxHeight: "75px",
                    objectFit: "cover",
                    borderRadius: "4px",
