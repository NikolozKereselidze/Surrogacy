...
export default function DonorsList({ donorType, title, apiEndpoint, }: DonorsListProps) {
    ...
    const applyFiltersAndSort = useCallback((donorsToFilter: Donor[]) => {
    ...
    return (<div className={styles.donorsPageContainer}>
      ...
      <div className={styles.donorTypeTabs}>
        {donorTypeTabs.map((tab) => (<button key={tab.id} className={`${styles.tabButton} ${pathname === tab.path ? styles.activeTab : ""}`} onClick={() => router.push(tab.path)} disabled={pathname === tab.path}>
            {tab.label}
          </button>))}
      </div>
      ...
      <div className={styles.filtersSection}>
        <div className={styles.filtersHeader}>
          ...
          <div className={styles.sortContainer}>
            <select id="sortSelect" name="sortSelect" value={`${sortOption.field}-${sortOption.direction}`} onChange={(e) => {
        ...
        {showFilters && (<div className={styles.filtersContainer}>
            <div className={styles.filterGroup}>
              <label htmlFor="ageRangeMin">
                Age Range: {filters.ageRange[0]} - {filters.ageRange[1]} years
              </label>
              <div className={styles.rangeInputs}>
                <input id="ageRangeMin" name="ageRangeMin" type="range" min="18" max="60" value={filters.ageRange[0]} onChange={(e) => setFilters((prev) => ({
                ...prev,
                ageRange: [parseInt(e.target.value), prev.ageRange[1]],
            }))}/>
                <input id="ageRangeMax" name="ageRangeMax" type="range" min="18" max="60" value={filters.ageRange[1]} onChange={(e) => setFilters((prev) => ({
                ...prev,
            ...
            <div className={styles.filterGroup}>
              ...
              <div className={styles.rangeInputs}>
                <input id="heightRangeMin" name="heightRangeMin" type="range" min="140" max="220" value={filters.heightRange[0]} onChange={(e) => setFilters((prev) => ({
                ...prev,
                heightRange: [
                ...
                <input id="heightRangeMax" name="heightRangeMax" type="range" min="140" max="220" value={filters.heightRange[1]} onChange={(e) => setFilters((prev) => ({
                ...prev,
                heightRange: [
                    prev.heightRange[0],
                    parseInt(e.target.value),
            ...
            <div className={styles.filterGroup}>
              ...
              <div className={styles.rangeInputs}>
                <input id="weightRangeMin" name="weightRangeMin" type="range" min="30" max="120" value={filters.weightRange[0]} onChange={(e) => setFilters((prev) => ({
                ...prev,
                weightRange: [
                    parseInt(e.target.value),
                    prev.weightRange[1],
                ...
                <input id="weightRangeMax" name="weightRangeMax" type="range" min="30" max="120" value={filters.weightRange[1]} onChange={(e) => setFilters((prev) => ({
                ...prev,
                weightRange: [
                    prev.weightRange[0],
                    parseInt(e.target.value),
            ...
            <div className={styles.filterGroup}>
              ...
              <select id="availability" name="availability" value={filters.available === null
                ? "all"
                : filters.available.toString()} onChange={(e) => setFilters((prev) => ({
                ...prev,
                available: e.target.value === "all"
                    ? null
                    : e.target.value === "true",
      ...
      {loading ? (<LoadingSpinner size="large"/>) : (<div className={styles.donorsGrid}>
        {filteredDonors.length === 0 ? (<div className={styles.noDonorsMessage}>
            <p>
              {donors.length === 0
                    ? `No ${donorType.replace("-", " ")} available at the moment.`
                    : `No ${donorType.replace("-", " ")} match your current filters.`}
            </p>
            <p>
              {donors.length === 0
          ...
          </div>) : (filteredDonors.map((donor) => (<div className={styles.donorCard} key={donor.id}>
              <div className={styles.donorImageContainer}>
                <Image className={styles.donorImage} src={getImageUrl(donor.databaseUser.mainImagePath)} alt={`${donorType.slice(0, -1)}`} width={138} height={138} loading="lazy"/>
              </div>
              <div className={styles.donorInfoWrapper}>
                ...
                <Button className={styles.viewProfileButton} onClick={() => {
