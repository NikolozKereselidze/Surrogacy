...
const normalizeLocale = (locale: unknown) => {
  if (locale === "ge") return "ka";
  return typeof locale === "string" ? locale : "en";
};

const publicMember = <T extends {
  translations: Array<{
    locale: string;
    role: string;
...
const isManagedImage = (imagePath: string | null | undefined) =>
  Boolean(imagePath && !imagePath.startsWith("/") && !imagePath.startsWith("http"));

const validationError = (res: Response, error: unknown) =>
  res.status(400).json({
    error: "Invalid team member data",
    details: error,
  });

const getPublicTeamMembers = async (req: Request, res: Response) => {
  try {
...
const getPublicTeamMember = async (req: Request, res: Response): Promise<any> => {
  try {
    const locale = normalizeLocale(req.query.locale);
...
const getAdminTeamMembers = async (_req: Request, res: Response) => {
  try {
    const members = await prisma.teamMember.findMany({
...
const createTeamMember = async (req: Request, res: Response): Promise<any> => {
...
const updateTeamMember = async (req: Request, res: Response): Promise<any> => {
  const parsed = teamMemberInputSchema.safeParse(req.body);
  if (!parsed.success) return validationError(res, parsed.error.flatten());
  ...
  try {
    ...
    if (current.imagePath !== member.imagePath && isManagedImage(current.imagePath)) {
      await deleteFileFromS3(current.imagePath).catch((error) =>
        console.error("Failed to delete replaced team image:", error),
      );
    }
    res.json(member);
  } catch (error) {
...
const deleteTeamMember = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (isManagedImage(member.imagePath)) {
      await deleteFileFromS3(member.imagePath).catch((error) =>
